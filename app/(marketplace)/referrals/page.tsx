...
export default function ReferralsPage() {
  ...
  const shareWhatsApp = () => {
    ...
    const text = encodeURIComponent(
      `🛍️ Join BATA — Nigeria's best campus marketplace!\nUse my referral link and let's both win:\n${stats.referralLink}`
    )
  ...
  const shareTelegram = () => {
    if (!stats) return
    const text = encodeURIComponent(
      `🛍️ Join BATA — Nigeria's best campus marketplace! Use my referral link: ${stats.referralLink}`
    )
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <Loader2 className="w-8 h-8 animate-spin text-bata-primary" />
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-16">
      ...
      {/* Hero Banner */}
      <div className="bg-gradient-to-br from-bata-primary via-purple-600 to-bata-secondary text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 -mt-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 mb-4 border border-gray-200">
            <span className="text-xl font-extrabold text-bata-primary tracking-widest flex-1">
              {stats.referralCode}
            </span>
            <button
              onClick={copyLink}
              className="flex items-center gap-1.5 text-sm font-semibold text-bata-primary hover:text-bata-dark transition-colors"
            >
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              { step: '3', text: 'When they complete an order, you earn 10% of the platform commission automatically' },
            ].map(item => (
              <div key={item.step} className="flex items-start gap-3">
                <div className="w-7 h-7 rounded-full bg-bata-primary text-white text-sm font-bold flex items-center justify-center flex-shrink-0">
                  {item.step}
                </div>
        ...
        {stats.recentRewards.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <Link href="/wallet" className="text-sm text-bata-primary font-medium flex items-center gap-1">
